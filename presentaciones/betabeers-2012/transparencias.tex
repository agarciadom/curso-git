...
\date[13 de julio de 2012]{13 de julio de 2012 \\[.1cm] \includegraphics[width=3cm]{betabeers-logo-white}}
...
\begin{frame}[fragile]{Clonar un repositorio}
  ...
  \begin{block}{Métodos de acceso}
    \begin{itemize}
    \item \url{git://}: anónimo, de sólo lectura, eficiente
    \item \fichero{rsync}: en desuso
    \item SSH: siempre cifrado y con autenticación
    \item HTTP(S) hasta v1.6.6.2: incómodo e ineficiente
    \item HTTP(S) desde v1.6.6.2: \foreignquote{english}{smart
        HTTP(S)} corrige estos problemas
    \end{itemize}
  \end{block}
...
\begin{frame}{Forjas con alojamiento Git}
  ...
  \begin{block}{Otra opción: Bitbucket (\url{http://bitbucket.org})}
    \begin{itemize}
    \item Gratis para proyectos libres, o cerrados con $<$ 5 miembros
...
\begin{frame}{Aspectos avanzados}
  ...
  \item \inlinecmd{instaweb}: visor Web (instalad desde fuentes)

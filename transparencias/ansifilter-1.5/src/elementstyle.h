...
class ElementStyle {
  ...
  /** \return True if italic */
   ...
   void setReset(bool b) {
	reset=b;
	if (reset){
		bold= italic= underline= conceal = blink = false;
		setFgColour("#000000");
		setBgColour("#ffffff");
		setFgColourID(0);
		setBgColourID(-1);
   ...
   /**   Set Background colour
         \param col colour of this element */
   void setBgColour(const StyleColour& col) {bgColour = col;}
   ...
   /**   Set Background colour
         \param col colour of this element */
   void setBgColour(const string& rgbVal) {bgColour.setRGB(rgbVal);}

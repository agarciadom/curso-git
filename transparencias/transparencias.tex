...
\newcommand{\showcommand}[1]{
  \runcommand{(#1) | ./filtrar.sh}
  {\small\ttfamily\input{cmd}}}
...
\begin{frame}{Creación de un repositorio}
  ...
  \orden{cd ejemplo}
  \orden{git init}
  \showcommand{cd ejemplo; git init}
\end{frame}

\begin{frame}{Estructura de un repositorio}
...
\begin{frame}[t]{Algunos de los ficheros en \fichero{.git}}
  ...
  \begin{overlayarea}{\textwidth}{6cm}
    \only<1>{
      \orden{cat config}
      \showcommand{cat ejemplo/.git/config}
    }
    \only<2>{
...
\begin{frame}{Nuestras primeras revisiones}
  Vamos a crear las 2 primeras revisiones de la rama \rama{master}:
  ...
  \showcommand{cd ejemplo; git config user.name Antonio; git config user.email a@b.com; git commit -m "primer commit"}
